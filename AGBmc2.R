...
AGBmc2 <- function(D, WD1, WD2 = NULL, H1, errH1 = NULL, H2 = NULL, errH2 = NULL, HDmodel2 = NULL, coord2 = NULL){
  ...
  if(!is.null(errH1)){
    ...
    h1l <- list()
    for(i in 1:1000){
      h1l[[i]] <- myrtruncnorm(length(D), H1, errH1, lower=1.3, upper=upper)
    }
  ...
  }else if(!is.null(HDmodel2) | !is.null(coord2)){
    H_2 <- predict_heights(D = D, model=HDmodel2, coord=coord2, plot=NULL)
  }else{
    H_2 <- H_1
  }
