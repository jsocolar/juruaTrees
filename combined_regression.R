...
regression_interpret_noTransect_model <- cmdstan_model("/Users/jacobsocolar/Dropbox/Work/Code/juruaTrees/regression_interpret_noTransect.stan")
...
for(j in 1:nrow(data_list)){
  ...
  for(i in 1:50){
    ...
    regression_interpret_data_i <- list(n_stem = nrow(rd2_i),
                                      ...
                                      agb_diff = (rd2_i$agb_diff - mean(rd2_i$agb_diff))/sd(rd2_i$agb_diff),
                                      dbh = (rd2_i$DBH - mean(rd2_i$DBH))/sd(rd2_i$agb_diff),
                                      wd_ratio = (rd2_i$wd_ratio_outer_inner - mean(rd2_i$wd_ratio_outer_inner))/sd(rd2_i$wd_ratio_outer_inner),
                                      transect = as.integer(as.factor(rd2_i$transect)),
                                      plot = as.integer(as.factor(rd2_i$plot)),
                                      species = as.integer(as.factor(rd2_i$species)),
                                      genus = as.integer(as.factor(rd2_i$genus)),
                                      family = as.integer(as.factor(rd2_i$family)))
    
    if(regression_interpret_data_i$n_plot > (regression_interpret_data_i$n_transect + 2)){
      regression_interpret_fit_i <- regression_interpret_model$sample(data = as.list(regression_interpret_data_i), chains = 1, iter_warmup = 600, 
                                                                      iter_sampling = 100, adapt_delta = .95)
    }else{
      regression_interpret_fit_i <- regression_interpret_noTransect_model$sample(data = as.list(regression_interpret_data_i), chains = 1, iter_warmup = 600, 
                                                                      iter_sampling = 100, adapt_delta = .95)
    }
